...
//const char *eyeCascadeFilename1="haarcascade_mcs_eyepair_big.xml";
//const char *eyeCascadeFilename2="haarcascade_eye.xml";
...
// Set to true to save the FaceRecognizer model before exiting.
bool saveModel= true;
...
void recognizeAndTrainUsingWebcam(VideoCapture &videoCapture, CascadeClassifier &faceCascade, CascadeClassifier &eyeCascade1, CascadeClassifier &eyeCascade2)
{
    ...
    if(resumeTrainingFromModel)
    {   //load the model
        cout<<"Resuming from previously trained model...\n";

        if (!strcmp(facerecAlgorithm, "FaceRecognizer.LBPH")==0)
         {
            cout<<"Can resume from model only using LBPH. Check the facerecAlgorithm. Exiting...\n";
            exit(0);
         }
        ...
        if(!model.empty())
        {
            ...
            modelParamReader>>m_numPersons;

            cout<<"Got [m_numPersons]="<<m_numPersons<<". Correct??\n";

            /*load the last-seen faces of the persons in the model */
         ...
         {
            m_mode=MODE_DETECTION;
            cout<<"Model is empty for some reason. Check it. Starting in Detection mode (from scratch)\n";
         }
    }
    ...
    while (true) {
        ...
        if (m_mode == MODE_DETECTION) {
            ...
            //Update the existing model if resumeTrainingFromModel is true.
            if (resumeTrainingFromModel)
            {
                if (strcmp(facerecAlgorithm, "FaceRecognizer.LBPH")==0)
                {
                    //Model is loaded before the while loop to set MODE_RECOGNITION on init.
                    //model->load(modelFileName);

                    if(model.empty())
                    {
                         cout<<"Loaded Model is empty. Training from scratch..\n";
                        resumeTrainingFromModel=false;
                        break;
                    }
                    if(!model.empty())
                    {   // Read the required model Parameters from file (modelParams.txt)
                        //m_numPersons=
                        //m_selectedPerson=
                        //m_latestFaces=
                        model=updateModelWithNewFaces(preprocessedFaces,faceLabels,model,facerecAlgorithm);
                        // Show the internal face recognition data, to help debugging.
                        if (m_debug)
                            showTrainingDebugData(model,faceWidth,faceHeight);
                        //Start recognizing faces after updating the face model
                        m_mode= MODE_RECOGNITION;
                    }
                ...
                {
                    cout<<"Only LBPH supports model update. Starting to train from scratch\n.";
                    resumeTrainingFromModel=false;
                }


            }
            //If resumeTrainingFromModel is not set or if the model is empty or if its not LBPH recognizer, train from scratch
            if(!resumeTrainingFromModel)
            {
...
int main(int argc, char *argv[])
{
    ...
    namedWindow(windowName,CV_GUI_EXPANDED); // Resizable window, might not work on Windows.
