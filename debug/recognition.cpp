...
Ptr<FaceRecognizer> learnCollectedFaces(const vector<Mat> preprocessedFaces, const vector<int> faceLabels, const string facerecAlgorithm)
{
    ...
    cout<<"\nStarting to train using the"<<faceLabels.size() << "collected faces...\n";
    double tic=cv::getTickCount();
    model->train(preprocessedFaces, faceLabels);
    double toc=cv::getTickCount();
    cout<<"\nFinished training the collected faces in"<<(toc-tic)/cv::getTickFrequency()<<"seconds\n";
...
Ptr<FaceRecognizer> updateModelWithNewFaces(const vector<Mat> preprocessedFaces,const vector<int> faceLabels, Ptr<FaceRecognizer> model, const string facerecAlgorithm)
{
    cout << "Updating the model with the new faces using the [" << facerecAlgorithm << "] algorithm ..." << endl;
   ...
   // model = Algorithm::create<FaceRecognizer>(facerecAlgorithm);
    if (model.empty()) {
        cerr << "ERROR: The FaceRecognizer algorithm [" << facerecAlgorithm << "] is not available in this version of OpenCV. Please update to OpenCV v2.4.1 or newer." << endl;
        exit(1);
    }

    // Do the actual training from the collected faces. Might take several seconds or minutes depending on input!
    cout<<"\nStarting to update using the"<<faceLabels.size() << "collected faces...\n";
    double tic=cv::getTickCount();
